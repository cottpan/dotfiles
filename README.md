...
| harlequin | カーソル移動とペイン移動を揃えた。`q` / `?` / `/` は下記の理由で不可 |
...
ペインの中でだけ効く操作（`results_viewer.*` / `data_catalog.*` / `code_editor.focus_*`）
だけを vim に寄せ、終了は `C-t` のままにしています（`C-q` は tmux の prefix に吸われるため）。
