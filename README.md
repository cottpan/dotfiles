...
| | キー |
| --- | --- |
| カーソル移動 | `j` / `k`（横があれば `h` / `l`） |
| ペイン移動 | `C-h` / `C-j` / `C-k` / `C-l`（tmux だけ prefix のあと `h` / `j` / `k` / `l`） |
| 閉じる | `q` |
| 検索 | `/` |
| ヘルプ | `?` |

ペイン移動を「アプリの中は `C-hjkl`、tmux は prefix + `hjkl`」で分けているのは、
どちらも押しっぱなしの手の形が同じで、かつ tmux が先に食べてしまわないためです。

| ツール | 状態 |
| --- | --- |
| Neovim | 基準。ウィンドウ移動は `C-hjkl`（診断は `[d` / `]d` に退避） |
| tmux | prefix `C-q` のあと `hjkl` でペイン移動。`C-q ?` でチートシート |
| hunk | 既定で揃っている（`g` / `G`、`[` `]` でハンク移動も vim 風） |
| yazi / gh-dash / glow | 既定で揃っている（yazi のヘルプだけ `?` を足した） |
| btop | `vim_keys = true` を入れて移動を `j` / `k` に |
| harlequin | カーソル移動とペイン移動を揃えた。`q` / `?` / `/` と一部の `C-h` / `C-j` は下記の理由で不可 |
...
素の `q` / `?` / `/` に置くと、エディタでその文字が打てなくなります。フォーカスのある
ペインの中でだけ効く操作（`results_viewer.*` / `data_catalog.*`）だけを vim に寄せ、
終了は `C-t` のままにしています（`C-q` は tmux の prefix に吸われるため）。

ペイン移動の既定は `F2`（エディタ）/ `F5`（結果）/ `F6`（ツリー）で、どれがどれか
覚えられないので方向に置き換えました。`C-k` / `C-l`（エディタへ）はどのペインからでも
効きますが、`C-j` と `C-h` には次の制約があります。

`C-j` は既定のキーマップ（vscode）が先に取っていて、重ねたキーマップからは奪えません
（binding は追加されるだけで、フォーカスに近いウィジェットのものが勝つ）。エディタでは
Run Query、ツリーでは Insert Name のままです。エディタの `C-j` は実行後にフォーカスが
結果へ移るので、結果的には「下のペインへ」動きます。

`C-h` は tmux の中では harlequin まで届きません。端末が送るのは `0x08` で、textual は
これを backspace として読みます（見分けるには kitty keyboard protocol か
modifyOtherKeys mode 2 が要りますが、textual が要求するのは前者、tmux 3.7 が解すのは
後者だけ）。結果の表では backspace に使い道が無いので、そこだけ backspace にも同じ移動を
割り当てて `C-h` を成立させています。エディタは 1 文字消す動作が勝ってどうにもならない
ので、そこからツリーへ行くキーは `C-d` に逃がしました（結果の表の `C-d` は内側にある
ページ送りが勝つので、そのままです）。
